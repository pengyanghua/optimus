...
def _params_distr(model):
    ...
    # adjust this to obtain better results, to balance communication overhead since the largest one has lower comm overhead
    OVERFLOWRATE = 1.0

    # if the size of a parameter block is too small (<0.1% of the model size), the main overhead is comm.
    SMALLSIZERATE = 1/(float(num_ps))/100
    ...
    def best_fit_alg(cap, cur_sizes, item_size):
        ...
        for size in cur_sizes:
            ...
            surplus = cap - size - item_size
            if surplus >= 0 and surplus < min_fit_size:
                min_fit_size = surplus
                server = cur_sizes.index(size)

        # can not be fitted, must overflow, place it to the bin with most empty space
        if server == -1:
    ...
    for key, elem in params_ps_map.items():
        # sort based on server id, this will lead to increased ps key required by ps-lite
